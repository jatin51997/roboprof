...
def process_files_in_directory(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        dirs[:] = [d for d in dirs if os.path.join(root, d) != processed_dir]  # Skip the processed directory

        # Get the relative path from the root directory
        rel_path = os.path.relpath(root, root_dir)

        # Determine the output directory
        output_dir = processed_dir if rel_path == '.' else os.path.join(processed_dir, rel_path)
